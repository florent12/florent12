# 👋 Salut, je suis Florent

 **Data Engineer | Cloud & Big Data Enthusiast | Data Engineer**

💡 Passionné par la **data** et le**Cloud**, j’aime construire des pipelines robustes, des APIs performantes et explorer la puissance du cloud pour des projets à forte valeur ajoutée.  

---

## 🚀 Projets phares

- 📡 **[Kafka Weather Streaming](lien)** – Producer/Consumer Kafka pour collecter et traiter des données météo en temps réel (Docker, Python).  
- 📊 **[Customer Data Platform (CDP)](lien)** – Orchestration d’audiences et activation multi-canaux (Salesforce, GA, Meta) avec FastAPI et GCP.  
- 🛣️ **[Waze-like Navigation App](lien)** – Application Firebase/Firestore pour améliorer la mobilité inclusive des PMR et agoraphobes.  
- 🤖 **[Azure AI Experiments](lien)** – Démonstrations d’IA (vision, NLP, traduction, voix) avec Azure Cognitive Services.  
- 📈 **[Data Science Hands-On](lien)** – Modélisation (classification & régression) en Python et R avec un dataset retail.  

---

## 🛠️ Stack technique

![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![Kafka](https://img.shields.io/badge/Kafka-231F20?logo=apachekafka&logoColor=white)
![GCP](https://img.shields.io/badge/GCP-4285F4?logo=googlecloud&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?logo=microsoftazure&logoColor=white)
![BigQuery](https://img.shields.io/badge/BigQuery-669DF6?logo=googlebigquery&logoColor=white)
![Firestore](https://img.shields.io/badge/Firestore-FFCA28?logo=firebase&logoColor=black)

---

## 📊 Stats GitHub

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=florent12&show_icons=true&theme=tokyonight)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?florent12=florent12&layout=compact&theme=tokyonight)  

---

## 🌍 Me retrouver

- 💼 [LinkedIn](https://www.linkedin.com/in/florent-atakora/)  
- ✉️ [Email](atakoraflorenr@gmail.com)  
